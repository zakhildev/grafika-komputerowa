...
Sphere::Sphere() { buildSphere(1, 12, 12); }
...
inline float Sphere::d2r(float deg) { return M_PI * deg / 180.0f; }

vec4 Sphere::generateSpherePoint(float r, float alpha, float beta) {
  alpha = d2r(alpha);
  beta = d2r(beta);
  return vec4(r * cos(alpha) * cos(beta), r * cos(alpha) * sin(beta),
              r * sin(alpha), 1.0f);
}
...
void Sphere::generateSphereFace(vector<vec4> &vertices,
  ...
  vertices.push_back(
      generateSpherePoint(r, alpha + step_alpha, beta + step_beta));
...
void Sphere::buildSphere(float r, float mainDivs, float tubeDivs) {
  ...
  for (int alpha = 0; alpha < round(tubeDivs); alpha++) {
    for (int beta = 0; beta < round(mainDivs); beta++) {
      ...
      generateSphereFace(face, faceVertexNormals, normal, r,
                         alpha * mult_alpha - 90.0f, beta * mult_beta,
...
void Sphere::drawSolid(bool smooth) { renderSolid(smooth, false); }
