...
Torus::Torus() { buildTorus(0.75, 0.25, 20, 20); }
...
inline float Torus::d2r(float deg) { return M_PI * deg / 180.0f; }

vec4 Torus::generateTorusPoint(float R, float r, float alpha, float beta) {
  alpha = d2r(alpha);
  beta = d2r(beta);
  return vec4((R + r * cos(alpha)) * cos(beta),
              (R + r * cos(alpha)) * sin(beta), r * sin(alpha), 1.0f);
}
...
void Torus::generateTorusFace(vector<vec4> &vertices,
                              vector<vec4> &vertexNormals, vec4 &faceNormal,
                              float R, float r, float alpha, float beta,
                              float step_alpha, float step_beta) {
  vertices.clear();
  vertexNormals.clear();
  ...
  vertices.push_back(
      generateTorusPoint(R, r, alpha + step_alpha, beta + step_beta));
  ...
  vertexNormals.push_back(
      computeVertexNormal(alpha + step_alpha, beta + step_beta));
...
void Torus::buildTorus(float R, float r, float mainDivs, float tubeDivs) {
  ...
  for (int alpha = 0; alpha < round(tubeDivs); alpha++) {
    for (int beta = 0; beta < round(mainDivs); beta++) {
      ...
      generateTorusFace(face, faceVertexNormals, normal, R, r,
                        alpha * mult_alpha, beta * mult_beta, mult_alpha,
                        mult_beta);
      ...
      for (int i = 0; i < 6; i++)
        internalFaceNormals.push_back(normal);
    }
  }
...
void Torus::drawSolid(bool smooth) { renderSolid(smooth, false); }
